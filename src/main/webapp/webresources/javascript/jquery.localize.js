...
(function() {
  ...
  normalizeCustId = function(custId){
	  if(!custId){
		  custId = 0;
	  }
	  return custId;
  ...
  $.localize = function(pkg, options) {
    ...
    loadLanguage = function(pkg, lang, cust_id, level) {
      var file;
      if (level == null) {
        level = 1;
	  }
      switch (level) {
        case 1:
          ...
          if (options.loadBase) {
            ...
            return jsonCall(file, pkg, lang, level, cust_id);
          } else {
            return loadLanguage(pkg, lang, cust_id, 2);
          }
          break;
        case 2:
          if (lang.length == 2) {
            file = "" + pkg + "-"+ cust_id + "." + fileExtension;
          }
		  else{
			  file = "" + pkg + "-"+ cust_id + "." + fileExtension;
		  }
	...
	// if no file, callback to Default cust id
    jsonCall = function(file, pkg, lang, level, cust_id) {
      ...
      successFunc = function(d) {
        ...
        //return loadLanguage(pkg, lang, cust_id, level + 1);
		if (typeof(Storage) != "undefined" && !localStorage.getItem("cust_"+cust_id)){
		  // Store
		  localStorage.setItem("cust_"+cust_id, JSON.stringify(d));
		  console.log(localStorage.getItem("cust_"+cust_id));
		}
      };
	  getDefaultJSON = function(d){
		  if(d.status == 404){
			  if(cust_id == 'DEFAULT'){
				  lang =  normaliseLang($.defaultLanguage);
			  }
			  if(lang != $.defaultLanguage){
				  loadLanguage(pkg, lang, "DEFAULT", level);
			  }
		  }
	  };
	  if(!localStorage.getItem("cust_"+cust_id)){
		  ajaxOptions = {
			url: file,
			dataType: "json",
			async: false,
		  ...
		  successFunc(eval('('+localStorage.getItem("cust_"+cust_id)+')'));
	  }
    };
    ...
    localizeElement = function(elem, key, value) {
      if (elem.is('input')) {
        ...
        localizeOptgroupElement(elem, key, value);
    	  ...
    	  localizeOptionsElement(elem, value);
      }
      else if (!$.isPlainObject(value)) {
        elem.html(value);
		if(elem.parent().attr('title')){
			elem.parent().attr('title', $.trim(elem.parent().text()));
		}
		else if(elem.attr('title')){
			elem.attr('title', $.trim(elem.text()));
		}
		if(elem.parent().find('img').attr('title')){
			elem.parent().find('img').attr('title', $.trim(elem.text())).attr('alt', $.trim(elem.text()));
		}
      }
    ...
    localizeOptionsElement = function(elem, value){
    	elem.text(value);
	...
	cust_id = normalizeCustId(options.custId);
    if (!(options.skipLanguage && lang.match(regexify(options.skipLanguage)))) {
      loadLanguage(pkg, lang, cust_id, 1);
    }
    return wrappedSet;
  };
