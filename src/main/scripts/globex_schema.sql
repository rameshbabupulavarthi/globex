...
/**new */
    CREATE TABLE `communication` (
      ...
      CONSTRAINT `COMMUNICATION_FILE_ID_FK` FOREIGN KEY (`FILE_ID`) REFERENCES `file` (`FILE_ID`),
      CONSTRAINT `COMMUNICATION_FROM_ORGAN_ID_FK` FOREIGN KEY (`FROM_ORGANIZATION_ID`) REFERENCES `organization` (`ORGANIZATION_ID`),
      CONSTRAINT `COMMUNICATION_FROM_USER_ID_FK` FOREIGN KEY (`FROM_USER_ID`) REFERENCES `t_user` (`ID`),
      CONSTRAINT `COMMUNICATION_TO_ORGANI_ID_FK` FOREIGN KEY (`TO_ORGANIZATION_ID`) REFERENCES `organization` (`ORGANIZATION_ID`)
    ) ENGINE=InnoDB AUTO_INCREMENT=12558 DEFAULT CHARSET=latin1;
    ...
    CREATE TABLE `alert` (
      ...
      KEY `ALERT_USER_ID_FK` (`USER_ID`),
      CONSTRAINT `ALERT_USER_ID_FK` FOREIGN KEY (`USER_ID`) REFERENCES `t_user` (`ID`)
    ) ENGINE=InnoDB AUTO_INCREMENT=1274 DEFAULT CHARSET=latin1;

    CREATE TABLE `exposure_data` (
      `EXPOSURE_DATA_ID` int(11) NOT NULL AUTO_INCREMENT,
      `APPLICATION_ID` int(11) NOT NULL,
      `COUNTRY` varchar(50) NOT NULL,
      `EXPOSURE` double(11,2) DEFAULT '0.00',
      `RATE` double(17,8) DEFAULT '0.00000000',
      `STATES` varchar(150) DEFAULT NULL,
      PRIMARY KEY (`EXPOSURE_DATA_ID`),
      KEY `EXPOSURE_DATA_APPLICATION_ID_FK` (`APPLICATION_ID`),
      CONSTRAINT `EXPOSURE_DATA_APPLICATION_ID_FK` FOREIGN KEY (`APPLICATION_ID`) REFERENCES `application` (`APPLICATION_ID`)
    ) ENGINE=InnoDB AUTO_INCREMENT=7243 DEFAULT CHARSET=latin1;

    CREATE TABLE `local_broker_insured_contact` (
      `LOCAL_BROKER_INSURED_CONTACT_ID` int(11) NOT NULL AUTO_INCREMENT,
      `APPLICATION_ID` int(11) NOT NULL,
      `COUNTRY` varchar(50) NOT NULL,
      ...
      `PHONE_AREA_CODE` int(5) DEFAULT NULL,
      `PHONE` bigint(20) DEFAULT NULL,
      `PHONE_EXTENSION` int(4) DEFAULT NULL,
      `FAX_COUNTRY_CODE` int(3) DEFAULT NULL,
      `FAX_AREA_CODE` int(5) DEFAULT NULL,
      `FAX` bigint(20) DEFAULT NULL,
      `EMAIL` varchar(50) DEFAULT NULL,
      `CONTACT_TYPE` int(1) NOT NULL,
      PRIMARY KEY (`LOCAL_BROKER_INSURED_CONTACT_ID`),
      KEY `LOCAL_BROKER_INSURED_CONTACT_APPLICATION_ID_FK` (`APPLICATION_ID`),
      CONSTRAINT `LOCAL_BROKER_INSURED_CONTACT_APPLICATION_ID_FK` FOREIGN KEY (`APPLICATION_ID`) REFERENCES `application` (`APPLICATION_ID`)
    ...
    CREATE TABLE `note` (
      ...
      `DATE_CREATED` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (`NOTE_ID`),
      KEY `NOTE_FILE_ID_FK` (`FILE_ID`),
      KEY `NOTE_ORGANIZATION_ID_FK` (`ORGANIZATION_ID`),
      KEY `NOTE_CREATED_BY_FK` (`CREATED_BY`),
      CONSTRAINT `NOTE_CREATED_BY_FK` FOREIGN KEY (`CREATED_BY`) REFERENCES `t_user` (`ID`),
      CONSTRAINT `NOTE_FILE_ID_FK` FOREIGN KEY (`FILE_ID`) REFERENCES `file` (`FILE_ID`),
      CONSTRAINT `NOTE_ORGANIZATION_ID_FK` FOREIGN KEY (`ORGANIZATION_ID`) REFERENCES `organization` (`ORGANIZATION_ID`)
